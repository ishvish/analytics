#Data analysis using dplyr package

library(dplyr)
dplyr::filter(mtcars, mpg>25 & am==1)
filter(mtcars, mpg>25 & am==1)
mtcars %>% filter(mpg>25 & am==1) %>% arrange(wt) %>% summarise(n())#pipe
?mtcars

mtcars %>% group_by(am) %>% summarise(avg=mean(mpg))
mtcars %>% group_by(gear) %>% summarise(avg=mean(wt))
